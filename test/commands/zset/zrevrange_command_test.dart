...
void main() {
  group('ZRevRangeCommand', () {
    ...
    test('should build the correct command with BYLEX', () {
      ...
      expect(command.commandParts, ['ZREVRANGE', 'myzset', '[z', '[a', 'BYLEX']);
    });

    test('should build the correct command with BYSCORE', () {
      final command = ZRevRangeCommand('myzset', '(5', '(1', byScore: true);
      expect(command.commandParts, ['ZREVRANGE', 'myzset', '(5', '(1', 'BYSCORE']);
    });

    test('should build the correct command with LIMIT', () {
      final command = ZRevRangeCommand('myzset', '0', '-1', limitOffset: 0, limitCount: 1);
      expect(command.commandParts, ['ZREVRANGE', 'myzset', '0', '-1', 'LIMIT', '0', '1']);
    });
    ...
    test('should apply prefix to key', () {
      ...
      expect(prefixedCommand.commandParts, ['ZREVRANGE', 'myprefix:myzset', '0', '-1']);
    });
  });
}
