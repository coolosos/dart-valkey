...
void main() {
  group('ZRevRangeWithScoresCommand', () {
    test('should build the correct command for basic ZREVRANGE WITHSCORES', () {
      ...
      expect(command.commandParts, ['ZREVRANGE', 'myzset', '0', '-1', 'WITHSCORES']);
    });

    test('should build the correct command with BYLEX', () {
      final command = ZRevRangeWithScoresCommand('myzset', '[z', '[a', byLex: true);
      expect(command.commandParts, ['ZREVRANGE', 'myzset', '[z', '[a', 'BYLEX', 'WITHSCORES']);
    });

    test('should build the correct command with BYSCORE', () {
      final command = ZRevRangeWithScoresCommand('myzset', '(5', '(1', byScore: true);
      expect(command.commandParts, ['ZREVRANGE', 'myzset', '(5', '(1', 'BYSCORE', 'WITHSCORES']);
    });

    test('should build the correct command with LIMIT', () {
      final command = ZRevRangeWithScoresCommand('myzset', '0', '-1', limitOffset: 0, limitCount: 1);
      expect(command.commandParts, ['ZREVRANGE', 'myzset', '0', '-1', 'LIMIT', '0', '1', 'WITHSCORES']);
    });
    ...
    test('should apply prefix to key', () {
      ...
      expect(prefixedCommand.commandParts, ['ZREVRANGE', 'myprefix:myzset', '0', '-1', 'WITHSCORES']);
    });
  });
}
