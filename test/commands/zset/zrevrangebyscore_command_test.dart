...
void main() {
  group('ZRevRangeByScoreCommand', () {
    test('should build the correct command for basic ZREVRANGEBYSCORE', () {
      ...
      expect(command.commandParts, ['ZREVRANGEBYSCORE', 'myzset', '+inf', '-inf']);
    });

    test('should build the correct command with WITHSCORES', () {
      final command = ZRevRangeByScoreCommand('myzset', '+inf', '-inf', withScores: true);
      expect(command.commandParts, ['ZREVRANGEBYSCORE', 'myzset', '+inf', '-inf', 'WITHSCORES']);
    });

    test('should build the correct command with LIMIT', () {
      final command = ZRevRangeByScoreCommand('myzset', '+inf', '-inf', limitOffset: 0, limitCount: 1);
      expect(command.commandParts, ['ZREVRANGEBYSCORE', 'myzset', '+inf', '-inf', 'LIMIT', '0', '1']);
    });
    ...
    test('should apply prefix to key', () {
      ...
      expect(prefixedCommand.commandParts, ['ZREVRANGEBYSCORE', 'myprefix:myzset', '+inf', '-inf']);
    });
  });
}
