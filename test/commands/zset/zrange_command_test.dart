...
void main() {
  group('ZRangeCommand', () {
    ...
    test('should build the correct command with LIMIT', () {
      final command = ZRangeCommand('myzset', '0', '-1', limitOffset: 0, limitCount: 1);
      expect(command.commandParts, ['ZRANGE', 'myzset', '0', '-1', 'LIMIT', '0', '1']);
    });

    test('should build the correct command with WITHSCORES', () {
      final command = ZRangeCommand('myzset', '0', '-1', withScores: true);
      expect(command.commandParts, ['ZRANGE', 'myzset', '0', '-1', 'WITHSCORES']);
    });
    ...
    test('should apply prefix to key', () {
      ...
      expect(prefixedCommand.commandParts, ['ZRANGE', 'myprefix:myzset', '0', '-1']);
    });
  });
}
